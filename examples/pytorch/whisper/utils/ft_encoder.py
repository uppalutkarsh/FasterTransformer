...
class FTWhisperEncoderWeight(object):
    def __init__(
            self,
            config,
            tensor_para_size,
    ...
    def load_from_model(self, model):  
        ...
        Weight loading order: PyTorch tensor order should conform to src/fastertransformer/th_op/WhisperEncoderOp.h:FasterTransformerWhisperEncoder. For per-layer weights, the tensor is a stack of the weight across all layers.
        '''
...
class FTWhisperEncoder(nn.Module):
    def __init__(self, encoder_weight_list, lib_path, head_num, head_size, inter_size, d_model, is_remove_padding,
                 ...
                 whisper_with_bias=True, mwhisper=False, position_embedding_type=1, activation_type="gelu", layernorm_type="post_layernorm"):
        super().__init__()
        ...
        try:
            self.encoder = torch.classes.FasterTransformer.WhisperEncoder(*encoder_weight_list, head_num, head_size, inter_size, d_model,
            ...
            self.encoder = torch.classes.FasterTransformerWhisperEncoder(*encoder_weight_list, head_num, head_size, inter_size, d_model,
                                                                    is_remove_padding, num_layer, num_bucket, max_distance, sparse, q_scaling, tensor_para_size, pipeline_para_size,
                                                                    whisper_with_bias, mwhisper, position_embedding_type, activation_type, layernorm_type)
