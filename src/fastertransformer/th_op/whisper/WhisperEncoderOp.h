...
class IFWhisperEncoder {
public:
    virtual ~IFWhisperEncoder() {}
...
template<typename T>
class FTWhisperEncoder: public IFWhisperEncoder {
public:
    FTWhisperEncoder(int                            head_num,
                  int                            head_size,
                  int                            inter_size,
                  int                            d_model,
                  int                            layer_num,
        ...
        _layernorm_type(layernorm_type)
    {
        ...
        whisper_encoder_weights.resizeLayer(_layer_num);
        whisper_encoder_weights.setWhisperStructureDiff(whisper_with_bias, mwhisper, use_gated_activation, position_embedding_type);
        ...
        for (int i = 0; i < _layer_num; i++) {
            ...
            whisper_encoder_weights.whisper_encoder_layer_weights[i]->attn_layernorm_weights_.gamma =
                get_ptr<T>(_weights[0]) + _d_model * (i - first_layer_index);
            whisper_encoder_weights.whisper_encoder_layer_weights[i]->attention_weights_.query_weight.kernel =
                get_ptr<T>(_weights[1]) + _d_model * hidden_dim / tensor_para_.world_size_ * (i - first_layer_index);
            whisper_encoder_weights.whisper_encoder_layer_weights[i]->attention_weights_.key_weight.kernel =
                get_ptr<T>(_weights[2]) + _d_model * hidden_dim / tensor_para_.world_size_ * (i - first_layer_index);
            whisper_encoder_weights.whisper_encoder_layer_weights[i]->attention_weights_.value_weight.kernel =
                get_ptr<T>(_weights[3]) + _d_model * hidden_dim / tensor_para_.world_size_ * (i - first_layer_index);
            whisper_encoder_weights.whisper_encoder_layer_weights[i]->attention_weights_.attention_output_weight.kernel =
                get_ptr<T>(_weights[4]) + hidden_dim / tensor_para_.world_size_ * _d_model * (i - first_layer_index);
            whisper_encoder_weights.whisper_encoder_layer_weights[i]->ffn_layernorm_weights_.gamma =
                get_ptr<T>(_weights[5]) + _d_model * (i - first_layer_index);
            whisper_encoder_weights.whisper_encoder_layer_weights[i]->ffn_weights_.intermediate_weight.kernel =
                get_ptr<T>(_weights[6]) + _d_model * _inter_size / tensor_para_.world_size_ * (i - first_layer_index);
            if (use_gated_activation) {
                whisper_encoder_weights.whisper_encoder_layer_weights[i]->ffn_weights_.intermediate_weight2.kernel =
                    get_ptr<T>(_weights[7])
                    + _d_model * _inter_size / tensor_para_.world_size_ * (i - first_layer_index);
            }
            whisper_encoder_weights.whisper_encoder_layer_weights[i]->ffn_weights_.output_weight.kernel =
                get_ptr<T>(_weights[8]) + _inter_size / tensor_para_.world_size_ * _d_model * (i - first_layer_index);
            if (_whisper_with_bias) {
                whisper_encoder_weights.whisper_encoder_layer_weights[i]->attn_layernorm_weights_.beta =
                    get_ptr<T>(_weights[13]) + _d_model * (i - first_layer_index);
                whisper_encoder_weights.whisper_encoder_layer_weights[i]->attention_weights_.query_weight.bias =
                    get_ptr<T>(_weights[14]) + hidden_dim / tensor_para_.world_size_ * (i - first_layer_index);
                whisper_encoder_weights.whisper_encoder_layer_weights[i]->attention_weights_.key_weight.bias =
                    get_ptr<T>(_weights[15]) + hidden_dim / tensor_para_.world_size_ * (i - first_layer_index);
                whisper_encoder_weights.whisper_encoder_layer_weights[i]->attention_weights_.value_weight.bias =
                    get_ptr<T>(_weights[16]) + hidden_dim / tensor_para_.world_size_ * (i - first_layer_index);
                whisper_encoder_weights.whisper_encoder_layer_weights[i]->attention_weights_.attention_output_weight.bias =
                    get_ptr<T>(_weights[17]) + _d_model * (i - first_layer_index);
                whisper_encoder_weights.whisper_encoder_layer_weights[i]->ffn_layernorm_weights_.beta =
                    get_ptr<T>(_weights[18]) + _d_model * (i - first_layer_index);
                whisper_encoder_weights.whisper_encoder_layer_weights[i]->ffn_weights_.intermediate_weight.bias =
                    get_ptr<T>(_weights[19]) + _inter_size / tensor_para_.world_size_ * (i - first_layer_index);
                if (use_gated_activation) {
                    whisper_encoder_weights.whisper_encoder_layer_weights[i]->ffn_weights_.intermediate_weight2.bias =
                        get_ptr<T>(_weights[20]) + _inter_size / tensor_para_.world_size_ * (i - first_layer_index);
                }
                whisper_encoder_weights.whisper_encoder_layer_weights[i]->ffn_weights_.output_weight.bias =
                    get_ptr<T>(_weights[21]) + _d_model * (i - first_layer_index);
            }
        }

        // Transformere Block-level weights
        whisper_encoder_weights.absolute_or_relative_position_embedding = get_ptr<T>(_weights[9]);
        whisper_encoder_weights.embedding_table                         = get_ptr<T>(_weights[10]);
        whisper_encoder_weights.pre_transformer_layernorm_weights.gamma = get_ptr<T>(_weights[11]);
        if (_mwhisper && _whisper_with_bias) {
        ...
        if (sparse) {
            ...
            ft::cublasMMWrapper cublas_wrapper = ft::cublasMMWrapper(_cublasHandle,
                                                                     ...
                                                                     nullptr);
            for (int i = 0; i < _layer_num; ++i) {
                whisper_encoder_weights.whisper_encoder_layer_weights[i]->compress_weights(cublas_wrapper, hidden_dim);
            }
        }
#endif
    }

    ~FTWhisperEncoder() override
    {
        FT_LOG_DEBUG(__PRETTY_FUNCTION__);
                 ...
                 bool                     removing_padding) override
    {
        ...
        ft::WhisperEncoder<T>* whisper_encoder = new ft::WhisperEncoder<T>(batch_size,
                                                                  seq_len,
                                                                  _head_num,
                                                                  _head_size,
                                                                  _inter_size,
        ...
        try {
            whisper_encoder->forward(&output_tensors, &input_tensors, &whisper_encoder_weights);
        }
...
private:
    ...
    ft::WhisperEncoderWeight<T> whisper_encoder_weights;

    ft::NcclParam tensor_para_;
    ft::NcclParam pipeline_para_;
};

class FasterTransformerWhisperEncoder: public th::jit::CustomClassHolder {
public:
    FasterTransformerWhisperEncoder(
...
private:
    ...
    int64_t                 d_model_;
    IFWhisperEncoder*          ft_whisper_encoder;
