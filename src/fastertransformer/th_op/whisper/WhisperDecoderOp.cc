...
#include "src/fastertransformer/th_op/whisper/WhisperDecoderOp.h"

namespace th = torch;
namespace torch_ext {

FasterTransformerWhisperDecoder::FasterTransformerWhisperDecoder(th::Tensor self_layernorm_gamma,
            ...
            layernorm_beta,       inter_bias,      inter_bias2,        output_bias}
{
    ...
    switch (_st) {
        case at::ScalarType::Float:
            ftdecoder = new FTWhisperDecoder<float>(head_num,
                                                    head_size,
                                                    inter_size,
                                                    d_model,
                                                    layer_num,
        ...
        case at::ScalarType::Half:
            ftdecoder = new FTWhisperDecoder<half>(head_num,
                                                   head_size,
                                                   inter_size,
                                                   d_model,
                                                   layer_num,
        ...
        case at::ScalarType::BFloat16:
            ftdecoder = new FTWhisperDecoder<__nv_bfloat16>(head_num,
                                                            head_size,
                                                            inter_size,
                                                            d_model,
                                                            layer_num,
...
std::vector<th::Tensor> FasterTransformerWhisperDecoder::forward2(int64_t step,
                                                                  th::Tensor from_tensor,
                                                                  th::Tensor memory_tensor,
                                                                  th::Tensor memory_sequence_length,
                                                                  th::Tensor sequence_length,
...
#ifdef LEGACY_THS
    torch::jit::class_<torch_ext::FasterTransformerWhisperDecoder>("FasterTransformerWhisperDecoder")
#else
    torch::jit::class_<torch_ext::FasterTransformerWhisperDecoder>("FasterTransformer", "WhisperDecoder")
#endif
        .def(torch::jit::init<th::Tensor,
                              ...
                              std::string,
                              std::string>())
        .def("forward", &torch_ext::FasterTransformerWhisperDecoder::forward)
        .def("forward2", &torch_ext::FasterTransformerWhisperDecoder::forward2);
