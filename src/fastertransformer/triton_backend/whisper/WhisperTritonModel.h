...
template<typename T>
struct WhisperTritonModel: public AbstractTransformerModel {

    WhisperTritonModel(size_t      tensor_para_size,
                    size_t      pipeline_para_size,
                    int         enable_custom_all_reduce,
                    std::string model_dir,
                    int         int8_mode);

    ~WhisperTritonModel() = default;
...
private:
    ...
    std::vector<std::shared_ptr<ft::WhisperEncoderWeight<T>>>  encoder_shared_weights_;
    std::vector<std::shared_ptr<ft::WhisperDecodingWeight<T>>> decoding_shared_weights_;
