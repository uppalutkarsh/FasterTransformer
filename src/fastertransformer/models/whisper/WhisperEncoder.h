...
protected:
    ...
    T*      whisper_encoder_emb_buf_    = nullptr;
    T*      whisper_encoder_in_buffer_  = nullptr;
    T*      attn_out_buf_            = nullptr;
    T*      whisper_encoder_out_buffer_ = nullptr;

    T* normed_from_tensor_  = nullptr;
    T* normed_attn_out_buf_ = nullptr;

public:
    WhisperEncoder(size_t                              max_batch_size,
                size_t                              max_seq_len,
                size_t                              head_num,
    ...
    WhisperEncoder(WhisperEncoder<T> const& whisper_layer);

    ~WhisperEncoder();

    void forward(TensorMap* output_tensors, TensorMap* input_tensors, const WhisperEncoderWeight<T>* whisper_encoder_weights);
