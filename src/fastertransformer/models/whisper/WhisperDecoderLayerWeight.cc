...
template<typename T>
WhisperDecoderLayerWeight<T>::WhisperDecoderLayerWeight(const size_t head_num,
    ...
    use_gated_activation_(use_gated_activation)
{
    ...
    real_weights_num_ = (11 + (use_gated_activation ? 1 : 0)) * (whisper_with_bias ? 2 : 1);

    FT_LOG_DEBUG("WhisperDecoderLayerWeight " + std::string(__func__) + " start");

    initialize();
    mallocWeights();
    setWeightPtr();

    FT_LOG_DEBUG("WhisperDecoderLayerWeight " + std::string(__func__) + " end");
}

template<typename T>
void WhisperDecoderLayerWeight<T>::initialize()
{
    FT_LOG_DEBUG("WhisperDecoderLayerWeight " + std::string(__func__) + " start");
...
template<typename T>
WhisperDecoderLayerWeight<T>::~WhisperDecoderLayerWeight()
{
    FT_LOG_DEBUG("WhisperDecoderLayerWeight " + std::string(__func__) + " start");
...
template<typename T>
WhisperDecoderLayerWeight<T>::WhisperDecoderLayerWeight(const WhisperDecoderLayerWeight& other):
...
template<typename T>
WhisperDecoderLayerWeight<T>& WhisperDecoderLayerWeight<T>::operator=(const WhisperDecoderLayerWeight& other)
{
    head_num_             = other.head_num_;
    size_per_head_        = other.size_per_head_;
    d_model_              = other.d_model_;
...
template<typename T>
void WhisperDecoderLayerWeight<T>::setWeightPtr()
{
...
template<typename T>
void WhisperDecoderLayerWeight<T>::mallocWeights()
{
    for (int i = 0; i < real_weights_num_; i++) {
...
template<typename T>
void WhisperDecoderLayerWeight<T>::loadModel(std::string dir_path, FtCudaDataType model_file_type)
{
    FT_LOG_DEBUG("WhisperDecoderLayerWeight " + std::string(__func__) + " start");
...
template<typename T>
void WhisperDecoderLayerWeight<T>::setWhisperWithBias(bool whisper_with_bias_para, bool use_gated_activation_para)
{
    whisper_with_bias_       = whisper_with_bias_para;
    use_gated_activation_ = use_gated_activation_para;
}

template struct WhisperDecoderLayerWeight<float>;
template struct WhisperDecoderLayerWeight<half>;
#ifdef ENABLE_BF16
template struct WhisperDecoderLayerWeight<__nv_bfloat16>;
#endif
