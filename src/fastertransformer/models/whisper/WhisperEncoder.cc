...
template<typename T>
void WhisperEncoder<T>::initialize()
{
...
template<typename T>
WhisperEncoder<T>::WhisperEncoder(WhisperEncoder<T> const& whisper_encoder):
    BaseLayer(whisper_encoder),
    max_batch_size_(whisper_encoder.max_batch_size_),
    max_seq_len_(whisper_encoder.max_seq_len_),
    head_num_(whisper_encoder.head_num_),
    size_per_head_(whisper_encoder.size_per_head_),
    inter_size_(whisper_encoder.inter_size_),
    d_model_(whisper_encoder.d_model_),
    hidden_units_(whisper_encoder.hidden_units_),
    num_layer_(whisper_encoder.num_layer_),
    num_bucket_or_max_seq_len_(whisper_encoder.num_bucket_or_max_seq_len_),
    max_distance_(whisper_encoder.max_distance_),
    sm_(whisper_encoder.sm_),
    q_scaling_(whisper_encoder.q_scaling_),
    attention_type_(whisper_encoder.attention_type_),
    sparse_(whisper_encoder.sparse_),
    activation_type_(whisper_encoder.activation_type_),
    layernorm_type_(whisper_encoder.layernorm_type_),
    tensor_para_(whisper_encoder.tensor_para_),
    pipeline_para_(whisper_encoder.pipeline_para_),
    custom_all_reduce_comm_(whisper_encoder.custom_all_reduce_comm_),
    enable_custom_all_reduce_(whisper_encoder.enable_custom_all_reduce_)
{
    initialize();
}
...
template<typename T>
void WhisperEncoder<T>::setStream(cudaStream_t stream)
{
...
void WhisperEncoder<T>::allocateBuffer()
{
    if (is_allocate_buffer_ == false) {
        ...
        whisper_encoder_emb_buf_ = (T*)allocator_->reMalloc(
            whisper_encoder_emb_buf_, sizeof(T) * max_batch_size_ * max_seq_len_ * d_model_, false);
        whisper_encoder_in_buffer_ = (T*)allocator_->reMalloc(
            whisper_encoder_in_buffer_, sizeof(T) * max_batch_size_ * max_seq_len_ * d_model_, false);
        attn_out_buf_ =
            (T*)allocator_->reMalloc(attn_out_buf_, sizeof(T) * max_batch_size_ * max_seq_len_ * d_model_, false);
        whisper_encoder_out_buffer_ = (T*)allocator_->reMalloc(
...
void WhisperEncoder<T>::allocateBuffer(size_t batch_size, size_t seq_len)
{
    ...
    whisper_encoder_emb_buf_ =
        (T*)allocator_->reMalloc(whisper_encoder_emb_buf_, sizeof(T) * batch_size * seq_len * d_model_, false);
    whisper_encoder_in_buffer_ =
        (T*)allocator_->reMalloc(whisper_encoder_in_buffer_, sizeof(T) * batch_size * seq_len * d_model_, false);
    attn_out_buf_ = (T*)allocator_->reMalloc(attn_out_buf_, sizeof(T) * batch_size * seq_len * d_model_, false);
    whisper_encoder_out_buffer_ =
...
void WhisperEncoder<T>::freeBuffer()
{
    if (is_allocate_buffer_) {
        ...
        allocator_->free((void**)(&whisper_encoder_emb_buf_));
        allocator_->free((void**)(&whisper_encoder_in_buffer_));
...
template<typename T>
bool WhisperEncoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool WhisperEncoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool WhisperEncoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int WhisperEncoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
                                ...
                                const WhisperEncoderWeight<T>* whisper_encoder_weights)
{
    ...
    invokeBuildRelativeAttentionBias(relative_attention_bias_,
                                     whisper_encoder_weights->absolute_or_relative_position_embedding,
                                     head_num_,
                                     request_seq_len,
    ...
    for (uint ite = 0; ite < iteration_num; ite++) {
        ...
        if (position_embedding_type == PositionEmbeddingType::absolute) {
            invokeInputIdsEmbeddingLookupPosEncoding(
                ...
                use_inputs_embeds ? input_tensors->at("inputs_embeds").getPtr<T>() :
                                    whisper_encoder_weights->embedding_table,
                whisper_encoder_weights->absolute_or_relative_position_embedding,
        ...
        switch (attention_type_) {
            case AttentionType::UNFUSED_MHA: {
                ...
                if (pipeline_para_.rank_ == 0) {
                    invokeRemovePadding(whisper_encoder_in_buffer_,
            ...
            case AttentionType::UNFUSED_PADDED_MHA: {
                ...
                if (use_inputs_embeds_buffer) {
                    cudaMemcpyAsync(whisper_encoder_emb_buf_,
                                    input_tensors->at("inputs_embeds").getPtrWithOffset<T>(d_model_offset),
            ...
            case AttentionType::FUSED_MHA: {
                ...
                // if (pipeline_para_.rank_ == 0) {
                //     invokeRemovePadding(
                //         whisper_encoder_in_buffer_, whisper_encoder_emb_buf_, padding_offset_, h_token_num, d_model_,
                //         stream_);
                //     sync_check_cuda_error();
                // }
                // sync_check_cuda_error();
            ...
            case AttentionType::FUSED_PADDED_MHA: {
                ...
                // whisper_encoder_input_ptr = whisper_encoder_emb_buf_ + d_model_offset;
                // whisper_encoder_output_ptr = output_tensors->at("output_hidden_state").getPtr<T>() + d_model_offset;
                // break;
            }
            default: {
...

            {
                ...
                attn_input_tensors.insertIfValid(
                    ...
                    Tensor{MEMORY_GPU,
                           ...
                           whisper_encoder_weights->position_embedding_type == PositionEmbeddingType::relative ?
                               relative_attention_bias_ :
                               nullptr});
        ...
        if (pipeline_para_.rank_ == pipeline_para_.world_size_ - 1) {
            if (mwhisper) {  // mBART has final layernorm after Transformer block. BART doesn't
                invokeGeneralT5LayerNorm(whisper_encoder_output_ptr,
                                         whisper_encoder_output_ptr,
                                         whisper_encoder_weights->post_transformer_layernorm_weights.gamma,
                                         whisper_encoder_weights->post_transformer_layernorm_weights.beta,
                                         layernorm_eps_,
                                         h_token_num,
                                         d_model_,
                                         stream_);
...
template class WhisperEncoder<half>;
#ifdef ENABLE_BF16
template class WhisperEncoder<__nv_bfloat16>;
#endif
