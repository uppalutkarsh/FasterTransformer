...
template<typename T>
void WhisperDecoder<T>::initialize()
{
...
template<typename T>
void WhisperDecoder<T>::allocateBuffer(size_t batch_size)
{
...
template<typename T>
bool WhisperDecoder<T>::isValidBatchSize(size_t batch_size)
{
...
template<typename T>
WhisperDecoder<T>::WhisperDecoder(WhisperDecoder<T> const& decoder):
...
template<typename T>
void WhisperDecoder<T>::setStream(cudaStream_t stream)
{
...
template<typename T>
bool WhisperDecoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool WhisperDecoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool WhisperDecoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int WhisperDecoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template class WhisperDecoder<half>;
#ifdef ENABLE_BF16
template class WhisperDecoder<__nv_bfloat16>;
#endif
