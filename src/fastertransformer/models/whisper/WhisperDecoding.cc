...
template<typename T>
void WhisperDecoding<T>::initialize()
{
    decoder_ = new WhisperDecoder<T>(0,  // max_batch_size_ * beam_width_,
                                     head_num_,
                                     size_per_head_,
                                     inter_size_,
                                     d_model_,
...
template<typename T>
void WhisperDecoding<T>::allocateBuffer()
{
    FT_CHECK(false);
}
...
template<typename T>
void WhisperDecoding<T>::freeBuffer()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void WhisperDecoding<T>::setStream(cudaStream_t stream)
{
...
template<typename T>
WhisperDecoding<T>::WhisperDecoding(size_t max_batch_size,
                                    size_t max_seq_len,
...
template<typename T>
WhisperDecoding<T>::WhisperDecoding(WhisperDecoding<T> const& decoding):
    BaseLayer(decoding),
...
template<typename T>
WhisperDecoding<T>::~WhisperDecoding()
{
    delete decoder_;
...
template<typename T>
void WhisperDecoding<T>::forward(TensorMap* output_tensors,
                                 TensorMap* input_tensors,
                                 const WhisperDecodingWeight<T>* decoding_weights)
{
    // input_tensors:
    ...
    for (int step = max_input_length; step <= (int)max_seq_len; step++) {
        ...
        for (uint ite = 0; ite < iteration_num; ++ite) {
            ...
            bool whisper_with_bias = decoding_weights->whisper_with_bias;
            bool mwhisper = decoding_weights->mwhisper;

            const cudaDataType_t gemm_data_type = getCudaDataType<T>();

            if (pipeline_para_.rank_ == pipeline_para_.world_size_ - 1) {
                T* decoder_output_buf_tmp = mwhisper ? normed_decoder_output_buf_ : decoder_output_buf_;
                ...
                if (tensor_para_.world_size_ == 1) {
                    float alpha = (!whisper_with_bias && tie_word_embeddings_) ? 1.0f / sqrt(d_model_) : 1.0f;
                    float beta = 0.0f;
#ifdef ENABLE_BF16
...
template class WhisperDecoding<float>;
template class WhisperDecoding<half>;
#ifdef ENABLE_BF16
template class WhisperDecoding<__nv_bfloat16>;
#endif
