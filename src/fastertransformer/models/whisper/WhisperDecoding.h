...
template<typename T>
class WhisperDecoding: public BaseLayer {
private:
    // meta data
...
public:
    WhisperDecoding(size_t                              max_batch_size,
                 size_t                              max_seq_len,
    ...
    WhisperDecoding(WhisperDecoding<T> const& WhisperDecoding);

    ~WhisperDecoding();

    void forward(TensorMap* output_tensors, TensorMap* input_tensors, const WhisperDecodingWeight<T>* Decoding_weights);
