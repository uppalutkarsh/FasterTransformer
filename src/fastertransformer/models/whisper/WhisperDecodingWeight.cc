...
#include "src/fastertransformer/models/whisper/WhisperDecodingWeight.h"
#include "src/fastertransformer/utils/logger.h"

namespace fastertransformer {

template<typename T>
WhisperDecodingWeight<T>::WhisperDecodingWeight(const size_t                head_num,
    ...
    position_embedding_type(pe_type)
{
    ...
    for (int l = 0; l < num_layer_; l++) {
        if (isValidLayerParallelId(l)) {
            decoder_layer_weights.push_back(new WhisperDecoderLayerWeight<T>(head_num_,
                                                                          size_per_head_,
                                                                          d_model_,
                                                                          inter_size_,
                                                                          mem_d_model_,
            ...
            decoder_layer_weights.push_back(new WhisperDecoderLayerWeight<T>());
        }
    }
    FT_LOG_DEBUG("WhisperDecodingWeight " + std::string(__func__) + " end");
}

template<typename T>
void WhisperDecodingWeight<T>::initialize()
{
    FT_LOG_DEBUG("WhisperDecodingWeight " + std::string(__func__) + " start");
...
template<typename T>
WhisperDecodingWeight<T>::~WhisperDecodingWeight()
{
    FT_LOG_DEBUG("WhisperDecodingWeight " + std::string(__func__) + " start");
...
template<typename T>
WhisperDecodingWeight<T>::WhisperDecodingWeight(const WhisperDecodingWeight& other):
    ...
    real_weights_num_(other.real_weights_num_)
{
    ...
    decoder_layer_weights.reserve(num_layer_);
    for (int l = 0; l < num_layer_; l++) {
        decoder_layer_weights.push_back(new WhisperDecoderLayerWeight<T>(*other.decoder_layer_weights[l]));
    }
    FT_LOG_DEBUG("WhisperDecodingWeight " + std::string(__func__) + " end");
}

template<typename T>
WhisperDecodingWeight<T>& WhisperDecodingWeight<T>::operator=(const WhisperDecodingWeight& other)
{
    head_num_                  = other.head_num_;
    size_per_head_             = other.size_per_head_;
    d_model_                   = other.d_model_;
...
template<typename T>
void WhisperDecodingWeight<T>::mallocWeights()
{
    FT_LOG_DEBUG("WhisperDecodingWeight " + std::string(__func__) + " start");
    for (int i = 0; i < real_weights_num_; i++) {
...
template<typename T>
void WhisperDecodingWeight<T>::setWeightPtr()
{
...
template<typename T>
void WhisperDecodingWeight<T>::loadModel(std::string dir_path)
{
    FT_LOG_DEBUG("WhisperDecodingWeight " + std::string(__func__) + " start");
...
template<typename T>
bool WhisperDecodingWeight<T>::isValidLayerParallelId(int l)
{
    FT_LOG_DEBUG("WhisperDecodingWeight " + std::string(__func__) + " start");
...
template<typename T>
void WhisperDecodingWeight<T>::resizeLayer(const int num_layer)
{
    FT_LOG_DEBUG("WhisperDecodingWeight " + std::string(__func__) + " start");
    decoder_layer_weights.clear();
    num_layer_ = num_layer;
...
template<typename T>
void WhisperDecodingWeight<T>::setWhisperStructureDiff(bool                  whisper_with_bias_para,
                                                 bool                  mwhisper_para,
                                                 ...
                                                 PositionEmbeddingType position_embedding_type_para)
{
    ...
    for (int i = 0; i < num_layer_; i++) {
        decoder_layer_weights[i]->setWhisperWithBias(whisper_with_bias_para, use_gated_activation_para);
    }
}

template struct WhisperDecodingWeight<float>;
template struct WhisperDecodingWeight<half>;
#ifdef ENABLE_BF16
template struct WhisperDecodingWeight<__nv_bfloat16>;
#endif
