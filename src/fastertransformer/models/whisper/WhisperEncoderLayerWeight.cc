...
template<typename T>
WhisperEncoderLayerWeight<T>::WhisperEncoderLayerWeight(const size_t head_num,
                                                  const size_t size_per_head,
                                                  const size_t d_model,
                                                  const size_t inter_size,
                                                  const size_t tensor_para_size,
                                                  const size_t tensor_para_rank,
                                                  const bool   whisper_with_bias,
    ...
    whisper_with_bias_(whisper_with_bias),
    use_gated_activation_(use_gated_activation)
{
    real_weights_num_ = (8 + (use_gated_activation_ ? 1 : 0))
                        * (whisper_with_bias_ ? 2 : 1);  // 8: Q, K, V, O, LayerNorm1, FC1, FC2, LayerNorm2
    FT_LOG_DEBUG("WhisperEncoderLayerWeight " + std::string(__func__) + " start");
    initialize();
    mallocWeights();
    setWeightPtr();
    FT_LOG_DEBUG("WhisperEncoderLayerWeight " + std::string(__func__) + " end");
}

template<typename T>
void WhisperEncoderLayerWeight<T>::initialize()
{
    FT_LOG_DEBUG("WhisperEncoderLayerWeight " + std::string(__func__) + " start");
...
template<typename T>
WhisperEncoderLayerWeight<T>::~WhisperEncoderLayerWeight()
{
    FT_LOG_DEBUG("WhisperEncoderLayerWeight " + std::string(__func__) + " start");
...
template<typename T>
WhisperEncoderLayerWeight<T>::WhisperEncoderLayerWeight(const WhisperEncoderLayerWeight& other):
    ...
    whisper_with_bias_(other.whisper_with_bias_),
...
template<typename T>
WhisperEncoderLayerWeight<T>& WhisperEncoderLayerWeight<T>::operator=(const WhisperEncoderLayerWeight<T>& other)
{
    FT_LOG_DEBUG("WhisperEncoderLayerWeight " + std::string(__func__) + " start");
...
template<typename T>
void WhisperEncoderLayerWeight<T>::compress_weights(cublasMMWrapper& cublas_wrapper, int hidden_dim)
{
    FT_LOG_DEBUG("WhisperEncoderLayerWeight " + std::string(__func__) + " start");
...
template<typename T>
void WhisperEncoderLayerWeight<T>::setWeightPtr()
{
    FT_LOG_DEBUG("WhisperEncoderLayerWeight " + std::string(__func__) + " start");
...
template<typename T>
void WhisperEncoderLayerWeight<T>::mallocWeights()
{
    FT_LOG_DEBUG("WhisperEncoderLayerWeight " + std::string(__func__) + " start");
    for (int i = 0; i < real_weights_num_; i++) {
...
template<typename T>
void WhisperEncoderLayerWeight<T>::loadModel(std::string dir_path, FtCudaDataType model_file_type)
{
    FT_LOG_DEBUG("WhisperEncoderLayerWeight " + std::string(__func__) + " start");
...
template<typename T>
void WhisperEncoderLayerWeight<T>::setWhisperWithBias(bool whisper_with_bias_para, bool use_gated_activation_para)
{
    whisper_with_bias_       = whisper_with_bias_para;
    use_gated_activation_ = use_gated_activation_para;
}

template struct WhisperEncoderLayerWeight<float>;
template struct WhisperEncoderLayerWeight<half>;
#ifdef ENABLE_BF16
template struct WhisperEncoderLayerWeight<__nv_bfloat16>;
#endif
